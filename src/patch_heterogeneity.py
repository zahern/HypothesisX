with open(r'C:\Users\ahernz\source\SearchLibrium\src\SearchLibrium\mixed_logit.py', 'r') as f:
    content = f.read()

# Find the insertion point - there are 4 newlines after # }
target = '        self.fn_generate_draws: DrawsFunction = self.generate_draws_halton if halton else self.generate_draws_random\n    # }\n\n\n\n    def _rebuild_index_arrays_for_reordered_varnames(self):'

replacement = '''        self.fn_generate_draws: DrawsFunction = self.generate_draws_halton if halton else self.generate_draws_random
...
if target in content:
    ...
    with open(r'C:\Users\ahernz\source\SearchLibrium\src\SearchLibrium\mixed_logit.py', 'w') as f:
        f.write(content)
    print('Replacement successful')
else:
    print('Target not found')
    idx = content.find('fn_generate_draws: DrawsFunction')
    if idx >= 0:
